...
STOPWORDS = {
    ...
    "and", "or", "but", "not", "if", "as", "it", "its",
    "this", "that", "what", "which", "who", "how", "when", "where",
    "tell", "give", "me", "please", "about", "need", "want",
    "also", "more", "than", "only", "any", "all", "some",
}

def keywords(text):
    # Keep words >= 3 chars, but also keep important 2-3 char acronyms
    short_ok = {"ecr", "ecnr", "boi", "eta", "oci", "pcc", "frro", "poe", "rpo", "mea", "icp", "isp"}
    return [
        w for w in re.findall(r"[a-z]+", text.lower())
        if (len(w) >= 3 and w not in STOPWORDS) or w in short_ok
    ]
...
#   [SECTION HEADER]
#   N. Entry Title • Field: value • Field: value
# ==========================================
...
# ==========================================
# FIELD ALIASES
# Maps user-query words to field labels
# that appear inside the chunk text.
# No domain words — purely structural labels.
# ==========================================
...
def extract_field_from_chunk(chunk, field):
    ...
    pattern = re.compile(
        r'(?:[•o\|]\s*)(?:' + label_pat + r')\s*:\s*(.+?)(?=\s*[•o\|]\s*(?:location|address|phone|email|contact|jurisdiction|in-charge|in charge)\s*:|$)',
...
def direct_qa_search(query):
    """
    Scan every chunk for Q:/A: or Ans: pairs.
    Score each question text against the user query.
    Return the single best answer, or None.
    """
    qwords = keywords(query)
    if not qwords:
        return None
    ...
    for chunk in chunks:
        # Normalize flat Q/A to newlines
        norm = re.sub(r'(?<!\n)\s+(?=Q\s*:)',  '\n', chunk)
        norm = re.sub(r'(?<!\n)\s+(?=A\s*:)',  '\n', norm)
        norm = re.sub(r'(?<!\n)\s+(?=Ans\s*:)', '\n', norm)
        ...
        for block in blocks:
            ...
            # Score question against query
            q_kw   = keywords(q_text)
            score  = len(set(qwords) & set(q_kw))

            # Bigram bonus
            for i in range(len(qwords) - 1):
                bigram = qwords[i] + " " + qwords[i + 1]
                if bigram in q_text.lower():
                    score += 3

            # Ratio bonus: fraction of query words found in question
            if score > 0 and q_kw:
                score += score / max(len(qwords), 1)
    ...
    # Require stricter match: at least 2 keyword hits, AND score must cover
    # a meaningful fraction of the query words (avoids single-word false positives)
    min_score = max(2, len(qwords) * 0.5)
    coverage  = best_score / max(len(qwords), 1)
    if best_score >= min_score and coverage >= 0.4 and best_answer:
        print(f"[QA-DIRECT] score={best_score:.2f} coverage={coverage:.2f} answer={best_answer[:60]}")
        return best_answer
    return None
    ...
    try:
        res = session.post(
            ...
            json={
                ...
                "prompt": prompt,
                "stream": False,
                "options": {"temperature": 0, "num_predict": 600, "top_k": 20, "top_p": 0.9}
            },
            timeout=90
        )
        ...
        not_found = ["not found in pdf", "no data found", "not present in", "not mentioned in"]
...
def find_section_chunks(query):
    ...
    matched = [
        ...
        if re.match(r'^\[(.+?)\]', chunk) and
           re.match(r'^\[(.+?)\]', chunk).group(1).lower() == best_header
    ]
...
def ask(query):
    ...
    try:
        ...
        print(
            "[ASK] Checking best chunk "
            "relevance before returning directly"
        )
        ...
        scored = (
            [(kw_score(qwords, c), c) for c in results]
            if qwords else []
        )

        if scored:

            best_kw_score, best = max(
                scored,
                key=lambda x: x[0]
            )
